...
function Alt_box({tableNumber, toShowStatus, C_R}){
  ...
  const display_notfilled = "Por favor complete la matriz para poder observar la razón de consistencia."
  ...
  //If the criteriaName comes as an Array, we can easily map over each element and create an variable that holds the thead structure.
   ...
   TableStruct(){
    ...
    <table className="tableStruct w-2/3 text-lg border-collapse border border-slate-500 bg-slate-300">
         ...
         <tbody>
          <tr>
            <td className="border font-semibold border-slate-600 bg-slate-300">Valores</td>
              {valueRow}
          </tr>
         </tbody>
    </table></div>) 
...
const EigenTable = new TableSingleRow(value, eigenNames, slicedEigen, "Vector propio");

const PWTable = new TableMultipleRow(value,eigenNames, PW_matrix.slice(1), "Tabla de comparación de criterios.");

// console.log(N_matrix.slice(2));
const N_matrixTable = new TableMultipleRow(value,eigenNames, N_matrix.slice(1) , "Matriz normalizada");
...
// const [selectChoice, setSelectChoice] = useState(new Map());
  ...
  const random_index_name = ["Igual importancia",
    "Igual a moderado",
    ...
    "Muy fuerte a extremo",
    "Importancia extrema",];
  const random_index_table = new TableSingleRow(9, random_index_name, random_index, "Referencia de peso de criterios");
  //   const [crit_radio]
  return (
    <div class="div-top" key={tableNumber}> 
    {random_index_table.TableStruct()}
      <div className="choice-title text-2xl font-serif font-semibold">Matriz de comparación</div> 
      <div class="table-container  div-top">
       
        <table className="matrix_table">
          <thead className="matrix_thead">
            <tr className="matrix_tr">
              <th className="matrix_th w-1/5" colSpan={2}>Más importancia</th>
              <th className="matrix_th" id="equal">Igual</th>
              <th className="matrix_th w-2/3" colSpan="10">¿Cuán es más importante?</th>
            </tr>
          </thead>
