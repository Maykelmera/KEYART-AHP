...
export default function Normal_matrix({ value, tableNumber }) {
  ...
  const postCriteriaData = () => {
    ...
    console.log(C_R);
    if(C_R < 0.1 & C_R >= 0)
    {alert('Datos guardados!'); setDisableButton(true);}
    else{
      alert('La razón de consistencia no es aceptable, debe ser mayor a 0 y menor a 0.1.');
      return;
    }  
  ...
  return (
    <div>
      ...
      <button className="rounded-md bg-cyan-200  w-96 h-10 px-1  my-6 text-sm font-medium
           ...
           onClick={postCriteriaData}>Enviar a la base de datos, cuando CR esté en verde </button>
